...
curl -w -k -L -c /tmp/cookie -b /tmp/cookie -X PUT -d '{"name":"'"$login"'","email":"root@localhost","password":"'"$new_password"'","role":"admin","expiration":"-1","pod":0,"pexpiration":"-1"}' -H 'Content-type: application/json' $endpoint/api/users/admin -o /tmp/web_passwd.out >/dev/null 2>&1
